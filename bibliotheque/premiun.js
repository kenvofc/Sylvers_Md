...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./recherche/donnees/role/boss.json", JSON.stringify(_dir));
};
...
const expiredPremiumCheck = (Kev, msg, _dir) => {
	setInterval(() => {
		let position = null;
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./recherche/donnees/role/boss.json", JSON.stringify(_dir));
			idny ? Kev.sendMessage(idny, { text: "Votre statut de boss est expirey veuillez le racheter." }) : "";
			idny = false;
		}
	}, 1000);
};
