...
exports.totalcase = () => {
   var file = fs.readFileSync("../SylversBot.js").toString()
...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./KenvFile/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./KenvFile/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./KenvFile/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./KenvFile/trash/${filename}.mp4`)
	Promise.all([unlink(`./KenvFile/video/${filename}.mp4`), unlink(`./KenvFile/gif/${filename}.gif`)])
	return buffer5
				   }
...
exports.smsg = (Kev, m, store) => {
  ...
  if (m.key) {
    ...
    if (m.isGroup) m.participant = Kev.decodeJid(m.key.participant) || '';
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.delete = () => Kev.sendMessage(m.quoted.chat, {
        delete: vM.key
      });
      m.quoted.copyNForward = (jid, forceForward = false, options = {}) => Kev.copyNForward(jid, vM, forceForward, options);
      m.quoted.download = () => Kev.downloadMediaMessage(m.quoted);
    }
  }
  if (m.msg.url) m.download = () => Kev.downloadMediaMessage(m.msg);
  ...
  m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? Kev.sendMedia(chatId, text, 'file', '', m, {
    ...options
  }) : Kev.sendText(chatId, text, m, {
    ...options
  });
  m.copy = () => exports.smsg(conn, M.fromObject(M.toObject(m)));
  m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => Kev.copyNForward(jid, m, forceForward, options);
  Kev.appenTextMessage = async (text, chatUpdate) => {
...
exports.GIFBufferToVideoBuffer = async (image) => {
    ...
    var buffer5 = await fs.readFileSync(`./KenvFile/trash/${filename}.mp4`)
    Promise.all([unlink(`./KenvFile/trash/${filename}.mp4`), unlink(`./KenvFile/trash/${filename}.gif`)])
    return buffer5
}